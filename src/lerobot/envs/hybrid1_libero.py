...
DEFAULT_DATA_ROOT = (
    Path(__file__).resolve().parents[3] / "data" / "mujoco_hybrid1_libero"
)
...
def _assets_root() -> Path:
    return Path(
        os.environ.get("LEROBOT_HYBRID1_LIBERO_ASSETS", DEFAULT_ASSETS_ROOT)
    ).expanduser().resolve()
...
class Hybrid1Toolbox(Hybrid1Object):
    ...
    def in_box(self, position, object_position):
        local = np.asarray(object_position) - np.asarray(position) - np.array(
            [0.0, 0.0, 0.105]
        )
...
class Hybrid1_Tabletop_Manipulation(  # noqa: N801
    ...
    def __init__(self, bddl_file_name, *args, **kwargs):
        self._hybrid1_target_object = read_hybrid1_bddl_inventory(
            bddl_file_name
        ).target_object
        kwargs["scene_xml"] = str(
            _assets_root() / "assets" / "arenas" / "hybrid1_tabletop.xml"
        )
...
REGION_SAMPLERS["hybrid1_tabletop_manipulation"] = REGION_SAMPLERS[
...
class Hybrid1LiberoEnv(Scene1LiberoEnv):
    def __init__(
        ...
        prompt: str = HYBRID1_PROMPT,
        scene_task: str = HYBRID1_TASK_ID,
        scene_variant: str = HYBRID1_VARIANT_ID,
        target_object: str = HYBRID1_TARGET_OBJECT,
        receptacle_object: str = HYBRID1_SUCCESS.receptacle_object,
        success_semantics: Mapping[str, Any] | None = None,
        **kwargs,
    ) -> None:
        os.environ["LEROBOT_HYBRID1_LIBERO_ASSETS"] = str(
            Path(assets_root).expanduser().resolve()
        )
        semantics = success_semantics or {
            name: getattr(HYBRID1_SUCCESS, name)
            for name in HYBRID1_SUCCESS.__dataclass_fields__
        }
        super().__init__(
            bddl_path=bddl_path,
    ...
    def _apply_original_scene_layout(self) -> None:
        ...
        sim = self._env.sim
        for object_name, position in HYBRID1_OBJECT_POSITIONS.items():
            joint_id = sim.model.joint_name2id(f"{object_name}_joint0")
            address = int(sim.model.jnt_qposadr[joint_id])
            sim.data.qpos[address : address + 3] = np.asarray(position)
            sim.data.qpos[address + 3 : address + 7] = np.array(
                [1.0, 0.0, 0.0, 0.0]
            )
        sim.forward()
        ...
        return {
            "hybrid1": {
                ...
                "success_semantics": {
                    ...
                    "minimum_lift_height_delta": (
                        self.success_semantics.minimum_lift_height_delta
                    ),
    ...
    *,
    ...
    def _make_env(spec: ResolvedHybrid1Spec, task_id: int, **kwargs):
        ...
        return Hybrid1LiberoEnv(
            ...
            success_semantics={
                name: getattr(spec.semantics, name)
                for name in spec.semantics.__dataclass_fields__
            },
            **kwargs,
        )
